...
#        self.fc1 = nn.Conv2d(num_feat, int(num_feat / 16), (1, 1), stride=1)
 #       self.relu = nn.ReLU()
  #      self.fc2 = nn.Conv2d(int(num_feat / 16), num_feat, (1, 1), stride=1)
    ...
    #    z = F.avg_pool2d(out, out.size()[2:])
        # excitation step
   #     s = torch.sigmoid(self.fc2(self.relu(self.fc1(z))))
     #   out = out + s.expand(out.size(0), out.size(1), out.size(2), out.size(3))
